...
Singleton {
    ...
    // Force an immediate write without waiting for the deferred timer.
    // Called on Component.onDestruction to prevent data loss on restart.
    function flush() {
        if (fileWriteTimer.running) {
            fileWriteTimer.stop();
        }
        configFileView.writeAdapter();
    }
    ...
    FileView {
        ...
        // Flush on destruction so a pending write is not lost when
        // Quickshell is restarted (e.g. after changing cornerStyle).
