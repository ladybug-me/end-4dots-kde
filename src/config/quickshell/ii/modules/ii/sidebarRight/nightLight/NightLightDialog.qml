...
WindowDialog {
    ...
    Column {
        ...
        ConfigSwitch {
            ...
            // Bind to actual state, but guard against feedback loops during preview
        ...
        WindowDialogSlider {
            ...
            // While dragging: live preview via KWin NightLight DBus
            // The preview works regardless of toggle state:
            //   - Toggle ON:  preview while dragging, commit on release
            //   - Toggle OFF: preview while dragging, revert on release (no persistent change)
            onMoved: {
                Hyprsunset.sliderPreview(value)
            }
            onPressedChanged: {
                if (!pressed) {
                    // Slider released — commit permanently
                    Hyprsunset.sliderReleased(value)
                    // Sync config value
                    Config.options.light.night.colorTemperature = value
                }
            }
            tooltipContent: Hyprsunset.isPreviewing && !Hyprsunset.temperatureActive
                ? `${Math.round(value)}K (preview only — Night Light is OFF)`
                : `${Math.round(value)}K`
        }
    }
