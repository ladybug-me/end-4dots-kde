...
pre_process() {
    ...
    if [ ! -d "$STATE_DIR"/user/generated/wallpaper ]; then
...
post_process() {
    ...
    # Set KDE Plasma wallpaper natively
...
switch() {
    ...
    "$ILLOGICAL_IMPULSE_VIRTUAL_ENV/bin/python" "$SCRIPT_DIR/generate_colors_material.py" "${generate_colors_material_args[@]}" \
