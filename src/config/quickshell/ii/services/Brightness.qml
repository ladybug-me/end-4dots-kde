...
// License: GPLv3
// KDE port: removed Hyprland.focusedMonitor dependency — brightness scroll now
// works on KDE Plasma 6 by using the bar's own screen context.
 ...
 * KDE port: Hyprland.focusedMonitor replaced with Quickshell.screens[0] as
 * primary/only screen fallback (suitable for laptops and single-monitor setups).
 * For multi-monitor, getMonitorForScreen(screen) is still used by bar widgets.
 */
Singleton {
    id: root
    ...
    // Get the primary monitor — first in list (KDE does not expose a focusedMonitor IPC)
    ...
    function increaseBrightness(): void {
        ...
        // KDE: use primary monitor (no focusedMonitor IPC on KDE)
    ...
    // KDE port: Anti-flashbang content adjustment disabled — it used Hyprland IPC
    // events (activewindowv2, workspacev2) which are not available on KDE/KWin.
    // Brightness adjustment via brightnessctl still works; the Connections block
    // is replaced with a no-op comment to preserve the file structure.
    //
    // Variants {
    //     model: Quickshell.screens
    //     ... (Hyprland IPC connections removed for KDE compatibility)
    // }
